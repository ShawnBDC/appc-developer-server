...
exports.convert = function (opts) {
	...
	var urlMatch, urlBase, urlBranch, urlDir;

	if (url) {
		urlMatch = url.match(/^(https:\/\/github\.com\/[^\/]+\/[^\/]+\/)blob\/([^\/]+)(.+)$/);
		...
		urlBase = urlMatch[1];
		urlBranch = urlMatch[2];
		urlDir = path.dirname(urlMatch[3]);
	}
	...
	// YouTube
	html = html.replace(/<p><a href="(https:\/\/www\.youtube\.com\/watch\?v=[^"]+)"><img[^>]+><\/a><\/p>/g, '<div class="embed-responsive embed-responsive-16by9">\n$1\n</div>');
	html = html.replace(/<p>(https:\/\/www\.youtube\.com\/watch\?v=[^<]+)<\/p>/g, '<div class="embed-responsive embed-responsive-16by9">\n$1\n</div>');
